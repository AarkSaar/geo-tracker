...
const USERS = {
  ...
  'user@geo.com':   { password: 'user123',  role: 'employee', name: 'John Doe' },
};
...
const fmtFull = d => new Date(d).toLocaleString([], { month:'short', day:'numeric', hour:'2-digit', minute:'2-digit' });
...
function tryLogin() {
  ...
  if ($('remember-me').checked) {
    localStorage.setItem(STORAGE_KEYS.SESSION, JSON.stringify({ email, role: user.role, name: user.name }));
  }
...
function loadDashboard() {
  ...
  const saved = JSON.parse(localStorage.getItem(STORAGE_KEYS.ACTIVE_SESS) || 'null');
  if (saved) {
...
function drawCanvas() {
  ...
  ctx.fillStyle = state.insideZone ? 'rgba(134,239,172,0.12)' : 'rgba(56,189,248,0.08)';
  ctx.fill();
  ...
  ctx.strokeStyle = state.insideZone ? 'rgba(34,197,94,0.3)' : 'rgba(239,68,68,0.3)';
  ctx.lineWidth = 1;
  ctx.stroke();
...
function setPosition(inside) {
  ...
  text.textContent = inside ? 'Status: WITHIN BOUNDARY' : 'Status: OUTSIDE BOUNDARY';
...
function clockIn() {
  ...
  localStorage.setItem(STORAGE_KEYS.ACTIVE_SESS, JSON.stringify({
...
function renderSessionLog() {
  ...
  container.innerHTML = html || '<p style="color:#475569;font-size:13px">No sessions today.</p>';
}
...
function exportCSV() {
  ...
  const rows = [['Employee', 'Clock In', 'Clock Out', 'Duration (min)']];
  records.forEach(r => rows.push([
    r.user, fmtFull(r.clockIn), fmtFull(r.clockOut),
...
$('auth-password').addEventListener('keydown', e => e.key === 'Enter' && tryLogin());
...
document.querySelectorAll('.sim-btn').forEach(btn => {
  btn.addEventListener('click', () => setPosition(btn.dataset.pos === 'inside'));
});
